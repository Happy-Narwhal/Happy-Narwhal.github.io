// Game scene
// -------------
var FRUIT_WARS_ASSET_BASE = '/games/Fruit_Wars/assets/';
function fwAsset(path) {
  return FRUIT_WARS_ASSET_BASE + path;
}

Crafty.scene('Game', function() {
  Crafty.background('url(' + fwAsset('unicorn.jpg') + ')');
...
Crafty.scene('Victory', function() {
	message = Crafty.e('2D, DOM, Text')
		...
		.textFont($text_css);
	Crafty.background('url(' + fwAsset('rainbowland.jpg') + ')');
...
Crafty.scene('Failure', function() {
	//Crafty.background('url(' + fwAsset('froggy.jpg') + ')');
	Crafty.background('url(' + fwAsset('applesauce.gif') + ')');
...
Crafty.scene('Loading', function(){
	...
	Crafty.load([
		fwAsset('frogs.png'),
    fwAsset('rainbowland.jpg'),
    fwAsset('fireball.png'),
    fwAsset('unicorn.jpg'),
    fwAsset('apple.png'),
    fwAsset('cthullu.png'),
    fwAsset('applesauce.gif'),
    fwAsset('chapstick.png'),
    fwAsset('candymountain.png'),
    fwAsset('bossapple.png'),
    fwAsset('tree2.png'),
    fwAsset('wizard.png'),
    fwAsset('divadance.m4a'),
		fwAsset('door_knock_3x.mp3'),
		fwAsset('door_knock_3x.ogg'),
		fwAsset('door_knock_3x.aac'),
    fwAsset('hadouken.mp3'),
		fwAsset('board_room_applause.mp3'),
		fwAsset('board_room_applause.ogg'),
		fwAsset('board_room_applause.aac'),
		fwAsset('candy_dish_lid.mp3'),
		fwAsset('candy_dish_lid.ogg'),
		fwAsset('candy_dish_lid.aac')
		], function(){

		Crafty.sprite(fwAsset('wizard.png'), {wizard:[0,0,42,52]});
		Crafty.sprite(fwAsset('tree2.png'), {spr_tree:[0,0,43,41]});
		Crafty.sprite(fwAsset('candymountain.png'), {candymountain:[0,0,64,59]});
		Crafty.sprite(fwAsset('chapstick.png'), {chapstick:[0,0,52,39]});
		Crafty.sprite(fwAsset('fireball.png'), {fireball:[0,0,28,32]});
		Crafty.sprite(fwAsset('apple.png'), {apple:[0,0,44,44]});
		Crafty.sprite(fwAsset('bossapple.png'), {bossapple:[0,0,88,88]});
		Crafty.sprite(fwAsset('cthullu.png'), {cthullu:[0,0,140,137]});
    
		Crafty.sprite(36, fwAsset('frogs.png'), {
			spr_frog:  [0, 0]
    });
 
		// Define our sounds for later use
		Crafty.audio.add({
      divadance: [fwAsset('divadance.m4a')],
      hadouken: [fwAsset('hadouken.mp3')],
			knock:    [fwAsset('door_knock_3x.mp3'), fwAsset('door_knock_3x.ogg'), fwAsset('door_knock_3x.aac')],
			applause: [fwAsset('board_room_applause.mp3'), fwAsset('board_room_applause.ogg'), fwAsset('board_room_applause.aac')],
			ring:     [fwAsset('candy_dish_lid.mp3'), fwAsset('candy_dish_lid.ogg'), fwAsset('candy_dish_lid.aac')]
		});
    score = 0;
    levelscore = 0;
    gold = 100;
